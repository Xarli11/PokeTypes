...
import { analyzeTeamDefense, getThreatAlerts } from './analysis.js?v=2.22.8';
...
let allPokemon = [];
let contrastData = {};
let appData = null; // Store full appData for analysis
...
export async function initProMode() {
    ...
    try {
        ...
        renderTeamGrid();
        initSimulator(); // Initialize the Interaction Checker
    } catch (e) {
    ...
    container.innerHTML = team.map((member, index) => {
        if (!member) {
            ...
            <div onclick="window.openSearchModal(${index})" class="team-slot-empty cursor-pointer bento-card dark:bg-slate-800 dark:border-slate-700 border-dashed border-2 border-slate-300 dark:border-slate-600 hover:border-indigo-500 dark:hover:border-indigo-400 flex flex-col items-center justify-center h-40 md:h-48 transition-all group">
                ...
                <span class="mt-2 text-xs font-bold text-slate-400 group-hover:text-indigo-500">Add Pokemon</span>
            </div>`;
        } else {
            ...
            <div class="team-slot-filled relative bento-card dark:bg-slate-800 dark:border-slate-700 p-2 flex flex-col items-center h-40 md:h-48 group">
                <button onclick="window.removeTeamMember(event, ${index})" class="absolute top-1 right-1 p-1 rounded-full text-slate-400 hover:bg-red-50 hover:text-red-500 transition-colors z-10" title="Remove">
...
function renderTeamAnalysis(team) {
    const analysisSection = document.querySelector('#view-pro section:last-child'); // The Analysis Placeholder section
    if (!analysisSection) return;

    if (team.every(p => p === null)) {
        analysisSection.innerHTML = `
            <div class="flex flex-col items-center justify-center py-12">
                <div class="p-4 rounded-full bg-indigo-50 dark:bg-indigo-900/30 mb-4">
                ...
                <h3 class="text-lg font-bold text-slate-800 dark:text-white mb-2">Team Analysis</h3>
                <p class="text-slate-500 dark:text-slate-400 max-w-md">Add Pokemon to your team to see detailed coverage analysis, weakness alerts, and offensive gaps.</p>
            </div>`;
        return;
    }
    ...
    let html = `
        <div class="p-6">
            <h2 class="text-xl font-bold text-slate-800 dark:text-white mb-6">Defense Coverage</h2>
    `;
    ...
    if (alerts.length > 0) {
        ...
        alerts.forEach(alert => {
            ...
            const message = alert.code === 'major_weakness' 
                ? `${alert.count} teammates are weak to ${alert.messageType}`
                : `No resistances to ${alert.messageType}`;
    ...
    appData.types.forEach(type => {
        ...
        const typeName = capitalizeWords(type); // Ensure localized display if needed, currently EN
        const pill = createTypePill(type, contrastData); // Contains localized name inside html
            ...
            <div class="flex flex-col gap-2 p-3 rounded-xl ${cardClass}">
                ...
                <div class="flex justify-between text-xs font-bold">
                    <div class="flex flex-col items-center flex-1">
                        <span class="text-red-500 mb-0.5">Weak</span>
                        <span class="${data.weak > 0 ? 'text-red-600 dark:text-red-400 text-lg' : 'text-slate-300'}">${data.weak || '-'}</span>
                    </div>
                    <div class="flex flex-col items-center flex-1 border-l border-r border-slate-200 dark:border-slate-700">
                        <span class="text-emerald-500 mb-0.5">Resist</span>
                        <span class="${data.resist > 0 ? 'text-emerald-600 dark:text-emerald-400 text-lg' : 'text-slate-300'}">${data.resist || '-'}</span>
                    </div>
                    <div class="flex flex-col items-center flex-1">
                        <span class="text-slate-400 mb-0.5">Immune</span>
...
window.removeTeamMember = (e, index) => {
    e.stopPropagation();
    if (confirm("Remove this Pokemon?")) {
...
function setupSearchModal() {
    ...
    const closeModal = () => {
        ...
        setTimeout(() => {
            ...
            input.value = '';
            resultsContainer.innerHTML = '<div class="py-12 text-center text-slate-400 text-sm">Type to start searching...</div>';
        }, 200);
    };
    ...
    input.addEventListener('input', (e) => {
        ...
        if (topMatches.length === 0) {
            resultsContainer.innerHTML = '<div class="py-12 text-center text-slate-400 text-sm">No Pokemon found.</div>';
        } else {
