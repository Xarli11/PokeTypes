...
| `type_calculate` | `displayAnalysis(t1, t2, t3)` runs with at least one real type selected, and the exact combination differs from the last one tracked. Re-renders of the *same* combo (e.g. a language toggle re-running `displayAnalysis` with unchanged types) do **not** refire it. | `type_1`, `type_2`, `type_3` (each a type name or `null`), `type_count` (1–3), `language` | A genuinely new type matchup was analyzed — single, dual, or triple. |
